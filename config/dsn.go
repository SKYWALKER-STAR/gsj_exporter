...
func (d *DSN) Setscheme(scheme string) {
	d.scheme = scheme
}

func (d *DSN) Setusername(username string) {
	fmt.Println("This is inside dsn username:",d.username)
	d.username = username
}

func (d *DSN) Setpassword(password string) {
	d.password = password
}

func (d *DSN) Sethost(host string) {
	d.host = host
}

func (d *DSN) Setport(port string) {
	d.port = fmt.Sprintf("inside port:%s",port)

	fmt.Println(d.port)
}

func (d *DSN) Setdbname(dbname string) {
	fmt.Println(dbname)
	d.dbname = dbname
}

func (d *DSN) Setpath(path string) {
	d.path = path
}
/*
func (d DSN) Setquery(query url.Values) {
	d.query = query
}
*/
...
func (d DSN) String() string {
	if d.password != "" {
		return fmt.Sprintf("%s://%s:%s@%s:%s/%s", d.scheme, d.username, d.password, d.host, d.port,d.dbname)
	}
