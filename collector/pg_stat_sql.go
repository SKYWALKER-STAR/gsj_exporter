...
func NewPGQueryCollector(config collectorConfig) (Collector, error) {
	return &PGQueryCollector{
		log: config.logger,
	}, nil
}
...
var (
	...
	pgSlowSQLTotal = prometheus.NewCounter(prometheus.CounterOpts {
		Namespace: namespace,
		Subsystem: QuerySubsystem,
		Name: "slow_sql_total",
		Help: "Total number of slow sql",
...
func (c PGQueryCollector) Update(ctx context.Context, instance *instance, ch chan<- prometheus.Metric) error {
	...
	for rows.Next() {
		...
		*/
		...
		if count.Float64 <= lastValue {
			pgSlowSQLTotal.Add(0)
		} else {
			incr := count.Float64 - lastValue
			pgSlowSQLTotal.Add(incr)
		}

		lastValue = count.Float64
