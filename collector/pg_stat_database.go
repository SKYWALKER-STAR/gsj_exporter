...
var (
	...
	*/
	statDatabaseDeadlocks = prometheus.NewCounter(prometheus.CounterOpts {
		Namespace: namespace,
		Subsystem: statDatabaseSubsystem,
		Name: "deadlocks",
		Help: "Number of deadlocks detected in this database",
	})

	oldDeadLocksNumber = 0.0
...
func (c *PGStatDatabaseCollector) Update(ctx context.Context, instance *instance, ch chan<- prometheus.Metric) error {
	...
	for rows.Next() {
		...
		//var activeTime sql.NullFloat64
		...
		if !datid.Valid {
			level.Debug(c.log).Log("Skipping collecting metric because it has no datid")
			continue
		}
		if !datname.Valid {
			level.Debug(c.log).Log("Skipping collecting metric because it has no datname")
			continue
		}
		if !numBackends.Valid {
			level.Debug(c.log).Log("Skipping collecting metric because it has no numbackends")
			continue
		}
		if !xactCommit.Valid {
			level.Debug(c.log).Log("Skipping collecting metric because it has no xact_commit")
			continue
		}
		if !xactRollback.Valid {
			level.Debug(c.log).Log("Skipping collecting metric because it has no xact_rollback")
			continue
		}
		if !blksRead.Valid {
			level.Debug(c.log).Log("Skipping collecting metric because it has no blks_read")
			continue
		}
		if !blksHit.Valid {
			level.Debug(c.log).Log("Skipping collecting metric because it has no blks_hit")
			continue
		}
		if !tupReturned.Valid {
			level.Debug(c.log).Log("Skipping collecting metric because it has no tup_returned")
			continue
		}
		if !tupFetched.Valid {
			level.Debug(c.log).Log("Skipping collecting metric because it has no tup_fetched")
			continue
		}
		if !tupInserted.Valid {
			level.Debug(c.log).Log("Skipping collecting metric because it has no tup_inserted")
			continue
		}
		if !tupUpdated.Valid {
			level.Debug(c.log).Log("Skipping collecting metric because it has no tup_updated")
			continue
		}
		if !tupDeleted.Valid {
			level.Debug(c.log).Log("Skipping collecting metric because it has no tup_deleted")
			continue
		}
		if !conflicts.Valid {
			level.Debug(c.log).Log("Skipping collecting metric because it has no conflicts")
			continue
		}
		if !tempFiles.Valid {
			level.Debug(c.log).Log("Skipping collecting metric because it has no temp_files")
			continue
		}
		if !tempBytes.Valid {
			level.Debug(c.log).Log("Skipping collecting metric because it has no temp_bytes")
			continue
		}
		if !deadlocks.Valid {
			level.Debug(c.log).Log("Skipping collecting metric because it has no deadlocks")
			continue
		}
		if !blkReadTime.Valid {
			level.Debug(c.log).Log("Skipping collecting metric because it has no blk_read_time")
			continue
		}
		if !blkWriteTime.Valid {
			level.Debug(c.log).Log("Skipping collecting metric because it has no blk_write_time")
			continue
		}
		/*
			if !activeTime.Valid {
				level.Debug(c.log).Log("Skipping collecting metric because it has no active_time")
				continue
			}
		*/
		statsResetMetric := 0.0
		if !statsReset.Valid {
			level.Debug(c.log).Log("No metric for stats_reset, will collect 0 instead")
		}
		if statsReset.Valid {
		...
		*/
		...
		if deadlocks.Float64 <= oldDeadLocksNumber {
			statDatabaseDeadlocks.Add(0)
		} else {
			incr := deadlocks.Float64 - oldDeadLocksNumber
			statDatabaseDeadlocks.Add(incr)
		}

		oldDeadLocksNumber = deadlocks.Float64
		ch <- statDatabaseDeadlocks
