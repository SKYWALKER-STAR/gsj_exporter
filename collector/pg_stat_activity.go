...
func NewPGStatActivityCollector(config collectorConfig) (Collector, error) {
...
func (c *PGStatActivityCollector) Update(ctx context.Context, instance *instance, ch chan<- prometheus.Metric) error {
	...
	for rows.Next() {
		...
		if !datname.Valid {
			level.Info(c.log).Log("Skipping collecting metric because it has no datid")
			continue
		}
		if !usename.Valid {
			level.Info(c.log).Log("Skipping collecting metric because it has no usename")
			continue
		}
		if !count.Valid {
			level.Info(c.log).Log("Skipping collecting metric because it has no count")
			continue
		}
		if !max_tx_duration.Valid {
			level.Info(c.log).Log("Skipping collecting metric because it has no max_tx_duration")
			continue
		}
